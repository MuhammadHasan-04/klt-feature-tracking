...
void _KLTSelectGoodFeatures(
    ...
    int ncols, int nrows, KLT_FeatureList featurelist,
    selectionMode mode)
{
     ...
     ============================================================ */
  {
    ...
    _KLT_FloatImage strength = _KLTCreateFloatImage(ncols, nrows);

    gpuComputeFeatureStrength(
        gradx, grady, strength,
    ...
    for (int y = bordery; y < nrows - bordery; y += tc->nSkippedPixels + 1)
      for (int x = borderx; x < ncols - borderx; x += tc->nSkippedPixels + 1)
      {
        *ptr++ = x;
        *ptr++ = y;
        *ptr++ = (int)strength->data[y * ncols + x];
        npoints++;
      }

    _KLTFreeFloatImage(strength);
  }
  ...
  _enforceMinimumDistance(
      ...
      ncols, nrows, tc->mindist, tc->min_eigenvalue,
